...
import { BitcoinCashService } from "./BitcoinCashService";
import { IBitcoinCashTransaction } from "./IBitcoinCashResponses";

export class BitcoinCashController implements ICryptoCurrency {

    private service: BitcoinCashService;

    constructor() {
    }

    public async onInit() {
        this.service = new BitcoinCashService(environment.localnodeConfigs.BitcoinCash);
    }
    ...
    public async listAccountTransactions(req: any) {
        ...
        const transactions: Array<IBitcoinCashTransaction> | undefined = await this.service.listAccountTransactions(account, options);
    ...
    public async listAccountDeposits(req: any) {
        ...
        const transactions: Array<IBitcoinCashTransaction> | undefined = await this.service.listAccountDeposits(account, page, offset);
        let standardizedTransactions: Array<ITransaction> = [];
        if (transactions) {
            standardizedTransactions = this.toRegularTransactions(transactions);
        }
    ...
    private toRegularTransactions(transactions: Array<IBitcoinCashTransaction>) {
